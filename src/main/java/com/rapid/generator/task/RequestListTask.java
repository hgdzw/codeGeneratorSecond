...
/**
 * @Classname RequestListTask
 * @Description 列表请求的task
 * @Date 2020/12/5 17:12
 * @Created by dongzhiwei
 */
public class RequestListTask extends AbstractTask {

    /**
     * 1.单表生成  2.多表时生成子表实体
     */
    public RequestListTask(String className, List<ColumnInfo> infos) {
     ...
     * 一对多关系生成主表实体
     */
    public RequestListTask(String className, String parentClassName, String foreignKey, List<ColumnInfo> tableInfos) {
     ...
     * 多对多关系生成主表实体
     */
    public RequestListTask(String className, String parentClassName, String foreignKey, String parentForeignKey, List<ColumnInfo> tableInfos) {
    ...
    public void run() throws IOException, TemplateException {
        ...
        entityData.put("EntityPackageName", ConfigUtil.getConfiguration().getPath().getRequest());
        ...
        String filePath = FileUtil.getSourcePath() + StringUtil.package2Path(ConfigUtil.getConfiguration().getPackageName()) + StringUtil.package2Path(ConfigUtil.getConfiguration().getPath().getRequest());
        String fileName = className + "ListRequest.java";
        // 生成Request文件
        FileUtil.generateToJava(FreemarketConfigUtil.TYPE_REQUESTLIST, entityData, filePath, fileName);
    }
}
