...
public class GeneratorUtil {
    ...
    {
//        sb.append("/**").append("\n");
        //MessageFormat.format("@ApiModelProperty(value = \"{0}\",required = false) ", info.getRemarks());
        sb.append(SPACE_4).append(MessageFormat.format("@ApiModelProperty(value = \"{0}\",required = false) ", info.getRemarks())).append("\n");
...
//          sb.append(tableName).append(".").append(infos.get(i).getColumnName()).append(" AS ").append("\"").append(infos.get(i).getPropertyName()).append("\",\n");
    ...
    public static String generateMapperResultMap(List<ColumnInfo> infos) {
        ...
        {
            i++;
            if(info.getColumnName()=="alert_remarks")
            {
                System.console().printf("alert_remarks"+i);
            }
            if (info.isPrimaryKey())
            {
                sb.append("<id column=\"").append(info.getColumnName()).append("\" jdbcType=\"").append(info.getType()).append("\" property=\"").append(info.getPropertyName()).append("\"/> \n");
            }
            else
            {
                sb.append(SPACE_8).append("<result column=\"").append(info.getColumnName()).append("\" jdbcType=\"").append(info.getType()).append("\" property=\"").append(info.getPropertyName()).append("\"/> \n");
            }
        }
        return sb.substring(0, sb.toString().length() - 2);
//        return sb.toString();
...
//            字符串判空 其他的不用
     ...
     * 生成Mapper 查询属性字段
     */
    public static String generateMapperSelectProperties(List<ColumnInfo> infos) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < infos.size(); i++) {
            if (i != 0) {
                sb.append(SPACE_8);
